...
function doRectanglesOverlap(rect1, rect2) {
  if (rect2.top > rect1.width || rect2.left > rect1.height
    || rect1.top > rect2.width || rect1.left > rect2.height) return false;
  return true;
}
...
function isInsideCircle(circle, point) {
  ...
  const d = Math.sqrt((centerX - x) ** 2 + (centerY - y) ** 2);
  return d < radius;
}
...
function isCreditCardNumber(ccn) {
  const arr = ccn.toString().split('').map((num) => Number(num));
  const sum = arr
    .map((item, i) => {
      if (i % 2 === arr.length % 2) {
        const pow = item * 2;
        return pow > 9 ? pow - 9 : pow;
      }
      return item;
    })
